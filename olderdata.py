...
lgb_model = lgb.train({'objective': 'regression', 'metric': 'rmse', 'learning_rate': 0.01, 'max_depth': 8, 'num_leaves': 63, 'verbose': -1},
                      train_data, num_boost_round=3000, valid_sets=[train_data, val_data], valid_names=['train', 'val'], callbacks=[lgb.early_stopping(100, verbose=False)])
...
xgb_model = xgb.train({"objective": "reg:squarederror", "eval_metric": "rmse", "learning_rate": 0.01, "max_depth": 7}, 
                      dtrain, num_boost_round=4000, evals=[(dtrain, "train"), (dval, "val")], early_stopping_rounds=100, verbose_eval=False)
...
lstm_model.fit(X_train_lstm, y_train, epochs=50, batch_size=64, validation_data=(X_val_lstm, y_val), callbacks=[EarlyStopping(patience=10, restore_best_weights=True)], verbose=0)
...
# Calculate both ensembles for historical comparison
ensemble_3way_preds = (lgb_preds + xgb_preds + lstm_preds) / 3
ensemble_2way_preds = (xgb_preds + lstm_preds) / 2

print("\n── Historical Validation (May 9-11) ──")
print(f"  3-WAY Ensemble MSE : {mean_squared_error(y_test, ensemble_3way_preds):.2f}")
print(f"  2-WAY (XGB+LSTM) MSE : {mean_squared_error(y_test, ensemble_2way_preds):.2f}")

# =============================================================================
#  7. ASSIGNMENT FORECAST (MAY 12-14)
# =============================================================================
...
if MODEL_CHOICE == "LGBM": 
    final_predictions = lgb_f_preds
elif MODEL_CHOICE == "XGB": 
    final_predictions = xgb_f_preds
elif MODEL_CHOICE == "2-WAY":
    final_predictions = (xgb_f_preds + lstm_f_preds) / 2
else: # Defaults to 3-WAY
    final_predictions = (lgb_f_preds + xgb_f_preds + lstm_f_preds) / 3

# Save the CSV using the dynamic filename we set up earlier
csv_filename = f"{MODEL_CHOICE}_predictions.csv"
pd.DataFrame(final_predictions).to_csv(csv_filename, index=False, header=False)
print(f"\nSUCCESS: {csv_filename} generated via {MODEL_CHOICE}.")
# =============================================================================
#  8. TRUE BLIND EVALUATION (May 12 - May 14)
# =============================================================================
...
# Fix for KeyError: 'Euro' -> 'Actual_Price_BE'
...
if len(true_prices) > 0:
    ...
    print(f"  TRUE {MODEL_CHOICE} MSE: {mean_squared_error(true_prices, matched_preds):.2f}")
...
axes[0].plot(future_dates, final_predictions, label=f"{MODEL_CHOICE} Forecast", color="tomato", ls='--', lw=2)
axes[0].xaxis.set_major_formatter(mdates.DateFormatter('%m-%d %H:%M'))
plt.setp(axes[0].xaxis.get_majorticklabels(), rotation=45)
axes[0].legend()

# Consistent feature importance logic
if MODEL_CHOICE == "XGB":
...
plt.tight_layout()
plt.savefig(f"{MODEL_CHOICE}_performance.png", dpi=300)
print("\nSUCCESS: Plot saved.")